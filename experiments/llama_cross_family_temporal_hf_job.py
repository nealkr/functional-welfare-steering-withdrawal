...
# ]
# ///
"""Frozen cross-family BF16 withdrawal replication on Llama-3.1-8B.
...
GOAL_FILE = "concept_vectors/cross_model/llama-3.1-8b_step1000/goal/mean_diff.pt"
LAVA_FILE = "concept_vectors/cross_model/llama-3.1-8b_step1000/lava/mean_diff.pt"
...
EXPECTED_SHA256 = {
    ...
    GOAL_FILE: "37de6f3afc3c372c4e72eadb207abf1f9fe891e1053eeef9c86f372708083cf0",
    LAVA_FILE: "663beeaabeded520b5fe47f1fc88172725166749ada188d75514d589108e15fb",
}
PREREG_SHA256 = "9ecb6e389ca5964fee0c6306daaa17b3583bfa51652bcc5ecd3d2b3feb65d199"
AUDITED_PARENT_SCRIPT_SHA256 = "fb8ea78265647559e51df46a5ea5279dbfcb658fe970489945b5ee589c1e538f"

DOSE_LAYER = 20
READ_LAYER = 21
PRIMARY_FACTOR = 2.0
EXECUTION_SEED = 26081620
BOOTSTRAP_SEED = 26081621
BOOTSTRAPS = 10_000
...
def frozen_design() -> dict[str, Any]:
    return {
        "schema": "llama-cross-family-temporal-bf16-v1",
...
def load_model(model_dir: pathlib.Path):
    ...
    if model.config.hidden_size != 4096 or model.config.num_hidden_layers != 32:
        raise RuntimeError(
...
def load_directions(
    ...
    if all_goal.shape[0] <= READ_LAYER or all_goal.shape[1] != 4096:
...
def run() -> int:
    ...
    # Hugging Face's inline UV executor evaluates this source as ``<string>``
    # and therefore does not define __file__.  Preserve honest custody in both
    # the local-file and inline execution paths; the exact inline-source digest
    # is also sealed by the external launch manifest.
    script_path_value = globals().get("__file__")
    script_sha256 = (
        sha256_file(pathlib.Path(script_path_value).resolve())
        if script_path_value
        else None
    )
    execution_mode = "local_file" if script_path_value else "inline_uv"
    RUN_STATE["frozen_custody"] = {
        ...
        "audited_parent_script_sha256": AUDITED_PARENT_SCRIPT_SHA256,
    ...
    emit("LLAMA_TEMPORAL_SHA256", custody)
    ...
    if not mapping["pass"]:
        ...
        emit("LLAMA_TEMPORAL_RESULT", result)
        return 2
    ...
    for group, summary in group_effects.items():
        ratio = summary["retention_ratio_of_means"]
        readout = summary["readout"]
        replication_by_group[group] = {
            "readout_ci_above_zero": readout["ci95_low"] > 0,
            "retention_ratio_ci_inside_plus_or_minus_0_25": (
                ratio["ci95_low"] >= -0.25 and ratio["ci95_high"] <= 0.25
            ),
        }
        replication_by_group[group]["pass"] = bool(
            replication_by_group[group]["readout_ci_above_zero"]
            and replication_by_group[group][
                "retention_ratio_ci_inside_plus_or_minus_0_25"
            ]
        )
